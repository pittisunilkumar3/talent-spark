const UserInterviewCalendarEvent = require('../models/user_interview_calendar_event.model');
const UserInterviewSchedule = require('../models/user_interview_schedule.model');
const { dbType } = require('../config/database');

// Get all user interview calendar events
exports.getAllCalendarEvents = async (req, res) => {
  try {
    ...
    if (dbType === 'mongodb') {
      ...
      total = await UserInterviewCalendarEvent.countDocuments(mongoQuery);
      calendarEvents = await UserInterviewCalendarEvent.find(mongoQuery)
        .sort({ created_at: -1 })
        .skip(offset)
      ...
      const result = await UserInterviewCalendarEvent.findAndCountAll(queryOptions);
    ...
    console.error('Error fetching user interview calendar events:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch user interview calendar events',
      error: error.message
    });
  }
};
...
exports.getCalendarEventById = async (req, res) => {
  try {
    ...
    if (dbType === 'mongodb') {
      // MongoDB implementation
      calendarEvent = await UserInterviewCalendarEvent.findById(id)
        .populate('interview_id');
    } else {
      // SQL implementation
      calendarEvent = await UserInterviewCalendarEvent.findByPk(id, {
        include: [
          {
            model: UserInterviewSchedule,
            as: 'interview'
...
exports.getCalendarEventsByInterviewId = async (req, res) => {
  try {
    ...
    if (dbType === 'mongodb') {
      // MongoDB implementation
      calendarEvents = await UserInterviewCalendarEvent.find({ interview_id: parseInt(interviewId) })
        .sort({ created_at: -1 });
    } else {
      // SQL implementation
      calendarEvents = await UserInterviewCalendarEvent.findAll({
...
exports.createCalendarEvent = async (req, res) => {
  try {
    ...
    let interview;
    if (dbType === 'mongodb') {
      interview = await UserInterviewSchedule.findById(interview_id);
    } else {
      interview = await UserInterviewSchedule.findByPk(interview_id);
    }
    ...
    let existingEvent;
    if (dbType === 'mongodb') {
      existingEvent = await UserInterviewCalendarEvent.findOne({
    ...
    if (dbType === 'mongodb') {
      // MongoDB implementation
      newCalendarEvent = new UserInterviewCalendarEvent({
        interview_id: parseInt(interview_id),
        calendar_type,
        event_id,
      ...
      // SQL implementation
      newCalendarEvent = await UserInterviewCalendarEvent.create({
        interview_id,
        calendar_type,
        event_id,
        organizer_id,
...
exports.updateCalendarEvent = async (req, res) => {
  try {
    ...
    if (dbType === 'mongodb') {
      // MongoDB implementation
      calendarEvent = await UserInterviewCalendarEvent.findByIdAndUpdate(
        id,
        updateData,
        { new: true }
      );
      ...
      // SQL implementation
      await UserInterviewCalendarEvent.update(
        updateData,
        { where: { id } }
      );
...
exports.deleteCalendarEvent = async (req, res) => {
  try {
    ...
    if (dbType === 'mongodb') {
      // MongoDB implementation
      result = await UserInterviewCalendarEvent.findByIdAndDelete(id);
    } else {
      // SQL implementation
      result = await UserInterviewCalendarEvent.destroy({
        where: { id }
      });
    }
