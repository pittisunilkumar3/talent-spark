...
exports.getAllDesignations = async (req, res) => {
  try {
    ...
    if (dbType === 'mongodb') {
      ...
      if (req.query.search) {
        const { Op } = Designation.sequelize;
        queryOptions.where = {
          ...queryOptions.where,
          [Op.or]: [
...
exports.updateDesignation = async (req, res) => {
  try {
    ...
    if (dbType === 'mongodb') {
      ...
      // Check if designation with same name already exists in the branch
      if (updateData.name && (updateData.branch_id || designation.branch_id)) {
        const existingDesignation = await Designation.findOne({ 
          name: updateData.name, 
          branch_id: updateData.branch_id || designation.branch_id,
          _id: { $ne: id }
        });
      ...
      if (updateData.name !== undefined || updateData.branch_id !== undefined) {
        ...
        const existingDesignation = await Designation.findOne({ 
          where: { 
            name, 
            branch_id: branch_id || null,
            id: { [Designation.sequelize.Op.ne]: parseInt(id) }
          } 
        });
